...
import com.dk.boruto.data.remote.FakeBorutoApi
...
class SearchHeroesSourceTest {
    ...
    fun `Search api with empty hero name, assert empty heroes list and LoadResult_Page`() =
        runTest {
            val nameQuery = ""
            val heroSource = SearchHeroesSource(borutoApi = borutoApi, query = nameQuery)
            ...
            val result = borutoApi.searchHeroes(name = nameQuery).heroes
            assertTrue { result.isEmpty() }
            assertTrue { loadResult is PagingSource.LoadResult.Page }
        }

    @Test
    fun `Search api with non-existant hero name, assert empty heroes list and LoadResult_Page`() =
        runTest {

            val nameQuery = "Unknown"
            val heroSource = SearchHeroesSource(borutoApi = borutoApi, query = nameQuery)
            val loadResult = heroSource.load(
                PagingSource.LoadParams.Refresh(
                    key = null,
